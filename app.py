...
st.markdown("""
    ...
    .mode-toggle:hover + .mode-popup, .mode-popup:hover {
        display: flex;
    }
    </style>
...
with st.container():
    mode_buttons_html = "".join([
        f"<button onclick=\"document.querySelector('[name=\\'mode\\']').value='{MODES[m]}'\">{m}</button>"
        for m in ["Chat", "Translate", "Poemify", "Songify", "Shakespearise", "Casualify", "Formalify", "Rephrase"]
        if m in MODES
    ])
        ...
        <div style="position: fixed; bottom: 0; left: 0; width: 100%; padding: 1rem; background: #1a1a1a; z-index: 9999;">
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <div style="position: relative;">
                    <div class="mode-toggle" style="font-size: 1.5rem; cursor: pointer;">➕</div>
